        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsValidData())
                {
                    ...
                    future[row, 0] = monthlyInvestment.ToString();
                    future[row, 1] = interestRateYearly.ToString();
                    future[row, 2] = years.ToString();
                    future[row, 3] = futureValue.ToString("n2");
                    row++;
                }
            }
            catch (Exception ex)
